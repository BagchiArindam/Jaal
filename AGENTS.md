...
- `unwrapToRepeatingItems` uses a 60%-dominant-tag threshold for grandchildren (relaxed from 100%), so Swiggy-style 2-column grids with occasional in-row ad slots (different tag) are correctly detected as 2D-matrix.
- `_filterAberrantSiblings` (Jaccard ≥ 0.15 vs median item) is applied to items after unwrap to reject brand-spotlight rows, carousels, and ad blocks before building the super-item or passing items to the AI.
- `buildSyntheticSuperItem(items)` walks every item's leaf nodes, groups by signature, and emits **up to 3** `<div class="jaal-super-item" data-sample="N">` blocks each with a different sample value per field — so the AI sees realistic value variation. Returns `{ html, htmlBlocks[], fieldCount, itemCount, leaves }`.
- `content-main` sends `htmlBlocks` (3 items) as the `samples` array to `/analyze`, and adds `unwrappedGrid`, `totalChildren`, `childTagDistribution` to metadata so the server prompt correctly identifies 2D vs 1D layouts.
...
- **Sort/filter** (`shared/sorter.js`): `sortElements` uses anchor-based reinsertion — records each item's next non-item sibling ("anchor") before detaching, then reinserts sorted items before their original anchor. Non-item siblings (brand spotlights, ad rows, row-wrapper divs) stay at their original positions. This fixes layout shatter on both 1D lists with mixed siblings (Amazon) and 2D-matrix grids (Swiggy Instamart).
- **Modal** (`extension/ui/modal.js`): position is determined by `setPreferredPos(nearElement)` called from `showLoading` in toolbar.js; the modal appears at the same location as the loading panel (near the picked element), not at fixed bottom-right. Close button destroys all toolbar instances (firing their `onClose` callbacks) and removes the modal host. `addToolbarTab()` both creates/shows the modal and reparents the toolbar hostEl — no need to call `openOrFocus()` first.
- **Popup re-open** (`extension/background.js`): `jaal-open-modal` from the popup re-injects PICKER_FILES and sends `jaal-auto-activate-multi` with matching configs for the current tab URL, so saved toolbars respawn when the user opens the panel from the browser-action popup.
